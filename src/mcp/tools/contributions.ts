...
import { ContributionKind, ContributionMode, RelationType, ScoreDirection } from "../../core/models.js";
...
const contributeInputSchema = z.object({
  ...
  artifacts: z
    ...
    .optional()
    .default({})
    .describe("Map of artifact name to content hash (blake3:<hex64>). Artifacts must already exist in CAS."),
  relations: z.array(relationSchema).optional().default([]).describe("Typed edges to other contributions"),
  ...
  context: z.record(z.string(), z.unknown()).optional().describe("Execution/evaluation context metadata"),
  agent: agentSchema.describe("Identity of the contributing agent"),
});
...
const reviewInputSchema = z.object({
  ...
  agent: agentSchema.describe("Reviewing agent identity"),
  metadata: z
...
const reproduceInputSchema = z.object({
  ...
  tags: z.array(z.string()).optional().default([]).describe("Tags"),
  context: z.record(z.string(), z.unknown()).optional().describe("Context metadata"),
  agent: agentSchema.describe("Reproducing agent identity"),
});
...
export function registerContributionTools(server: McpServer, deps: McpDeps): void {
  // --- grove_contribute ---------------------------------------------------
  server.registerTool("grove_contribute", {
    description:
    ...
    try {
      ...
      const input: ContributionInput = {
        ...
        ...(args.context !== undefined
          ...
          : {}),
        agent: args.agent as AgentIdentity,
        createdAt: now,
      };
  ...
  // --- grove_review -------------------------------------------------------
  server.registerTool("grove_review", {
    description:
  ...
  // --- grove_reproduce ----------------------------------------------------
  server.registerTool("grove_reproduce", {
    description:
